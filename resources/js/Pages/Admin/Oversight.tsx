...
export default function Oversight({
    ...
    useEffect(() => {
        ...
        if (flaggedTransactions.length > 0) {
            setLastTransactionId(
                Math.max(...flaggedTransactions.map((t) => t.id)),
            );
        }
        ...
        const pollInterval = setInterval(async () => {
            try {
                ...
                if (data.new_documents?.length > 0) {
                    toast.success(
                        `${data.new_documents.length} new KYC document(s) uploaded`,
                    );
                    ...
                    setStats((prev) => ({
                        ...prev,
                        pending_kyc_count:
                            prev.pending_kyc_count + data.new_documents.length,
                    }));
                }

                if (data.new_transactions?.length > 0) {
                    toast.warning(
                        `${data.new_transactions.length} new flagged transaction(s)`,
                    );
                    ...
                    setStats((prev) => ({
                        ...
                        flagged_transactions_count:
                            prev.flagged_transactions_count +
                            data.new_transactions.length,
                    }));
                }
            } catch (error) {
    ...
    const handleViewDocument = async (doc: PendingDocument) => {
        ...
        try {
            const response = await fetch(
                `/admin/oversight/documents/${doc.id}`,
                {
                    headers: {
                        'X-CSRF-TOKEN':
                            document
    ...
    const handleApproveKYC = async (documentId: number) => {
        try {
            const response = await fetch(
                `/admin/oversight/kyc/${documentId}/approve`,
                {
                    method: 'POST',
                    headers: {
            ...
            if (response.ok) {
                ...
                setStats((prev) => ({
                    ...prev,
                    pending_kyc_count: prev.pending_kyc_count - 1,
                }));
            } else {
    ...
    const handleRejectKYC = async () => {
        ...
        try {
            const response = await fetch(
                `/admin/oversight/kyc/${selectedDocument.id}/reject`,
                {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
    ...
    const handleResolveFraud = async (
        ...
        try {
            const response = await fetch(
                `/admin/oversight/fraud/${transactionId}/resolve`,
                {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
            ...
            if (response.ok) {
                ...
                setStats((prev) => ({
                    ...prev,
                    flagged_transactions_count:
                        prev.flagged_transactions_count - 1,
                }));
            } else {
    ...
    const handleBlockUser = async () => {
        ...
        try {
            const response = await fetch(
                `/admin/oversight/user/${selectedUser.id}/block`,
                {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
                ...
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-muted-foreground">
                            Real-time KYC and fraud monitoring
                        </p>
                    </div>
                    ...
                    <div className="rounded-lg border bg-card p-6">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-2xl font-bold">
                                    {stats.flagged_transactions_count}
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                    <div className="w-full max-w-md rounded-lg bg-background p-6 shadow-lg">
                        ...
                        <p className="mt-2 text-sm text-muted-foreground">
                            You are rejecting the document for{' '}
                            <strong>{selectedDocument.user_name}</strong>.
                            Please provide a reason that will be sent to the
                            user.
                        </p>
                        <textarea
                            value={rejectReason}
                            ...
                            className="mt-4 min-h-[100px] w-full rounded-md border px-3 py-2 text-sm"
                        />
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                    <div className="w-full max-w-md rounded-lg bg-background p-6 shadow-lg">
                        ...
                        <p className="mt-2 text-sm text-muted-foreground">
                            ...
                            will prevent them from accessing their account.
                        </p>
                        <div className="mt-4 rounded-md bg-red-50 p-3 text-sm text-red-800">
                            This action cannot be easily undone. The user will
                            need to contact support to restore their account.
                        </div>
                        <div className="mt-4 flex justify-end gap-2">
                            <button
                                onClick={() => {
            ...
            <DocumentModal
                ...
                onApprove={async () => {
                    if (selectedDocument) {
                        await handleApproveKYC(selectedDocument.id);
                        setShowDocumentModal(false);
                        setDocumentUrl(null);
                        setSelectedDocument(null);
                    }
